...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'wp_frappe_data_store_' . $page_slug_underscore . '_routes_data';
 ...
 */
function wp_frappe_data_store_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'wp-frappe-data-store/routes/' . $route['name'] . '/content';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'wp-frappe-data-store/routes/' . $route['name'] . '/route';
				$extension = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '.js' : '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for frappe-data-store
/**
 * Register routes for frappe-data-store page (full-page mode).
 */
function wp_frappe_data_store_register_frappe_data_store_page_routes() {
	global $wp_frappe_data_store_frappe_data_store_routes_data;
	wp_frappe_data_store_register_page_routes( $wp_frappe_data_store_frappe_data_store_routes_data, 'wp_frappe_data_store_register_frappe_data_store_route' );
}
add_action( 'frappe-data-store_init', 'wp_frappe_data_store_register_frappe_data_store_page_routes' );

/**
 * Register routes for frappe-data-store page (wp-admin mode).
 */
function wp_frappe_data_store_register_frappe_data_store_wp_admin_page_routes() {
	global $wp_frappe_data_store_frappe_data_store_routes_data;
	wp_frappe_data_store_register_page_routes( $wp_frappe_data_store_frappe_data_store_routes_data, 'wp_frappe_data_store_register_frappe_data_store_wp_admin_route' );
}
add_action( 'frappe-data-store-wp-admin_init', 'wp_frappe_data_store_register_frappe_data_store_wp_admin_page_routes' );
